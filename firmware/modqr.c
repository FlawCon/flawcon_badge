...
STATIC mp_obj_t qr_matrix(mp_obj_t self_in) {
    ...
    for (uint8_t y = 0; y < (self->qrCode.size+2); y++) {
        mp_obj_list_store(top_row, mp_obj_new_int(y), mp_const_true);
    }
    ...
    for (uint8_t y = 0; y < self->qrCode.size; y++) {
        ...
        mp_obj_list_store(row, mp_obj_new_int(0), mp_const_true);
        ...
        mp_obj_list_store(row, mp_obj_new_int(self->qrCode.size+1), mp_const_true);
    }
    ...
    for (uint8_t y = 0; y < (self->qrCode.size+2); y++) {
        mp_obj_list_store(bottom_row, mp_obj_new_int(y), mp_const_true);
    }
